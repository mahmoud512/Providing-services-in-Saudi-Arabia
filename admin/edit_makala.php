  ...
  <?php
        ...
        ?>
                  <?php
                    ...
                    $select_serv = "SELECT * FROM `makala` WHERE id =$id ";
                    $result = $con->query($select_serv);
                    foreach($result as $value){
                  ?>
  <div class="container-fluid">
    <div class="content" style="padding: 20px 20px; background-color: #f3f3f3; border-radius: 20px;">
      <form action="layout/function/edit_makala.php" method="post" enctype="multipart/form-data">
        <input type="hidden" value="<?=$value['id']?>" name="id">
        <label for="nametitle">عنوان المقالة : </label>
        <input type="text" name="title" value="<?=$value['title']?>" id="nametitle" class="form-control">
        <span class="text-danger"><?php e('title') ?></span>
        <br>
        <label for="dec"> محتوي المقالة : </label>
        <div class="mb-3">
          <textarea class="textarea" name="desc" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?=$value['desc']?></textarea>
          <span class="text-danger"><?php e('desc') ?></span>
        </div>
        <br>
        <img style="width: 200px; height: 200px;" src="assets/img/<?=$value['cover']?>" alt="">
        <br>
